...
def save_disp(disp:torch.Tensor , filepath):
    disp = disp.cpu().numpy()
    padder = InputPadder(disp.shape, divis_by=32)
    disp = padder.unpad(disp)
    disp = np.round(disp * 256).astype(np.uint16)
    cv2.imwrite(filepath, disp.squeeze())

def save_disp_color(disp:torch.Tensor , filepath):
    disp = disp.cpu().numpy()
    padder = InputPadder(disp.shape, divis_by=32)
    disp = padder.unpad(disp)
    disp = np.round(disp * 256).astype(np.uint16)
    cv2.imwrite(filepath, cv2.applyColorMap(cv2.convertScaleAbs(disp.squeeze(), alpha=0.01),cv2.COLORMAP_INFERNO), [int(cv2.IMWRITE_PNG_COMPRESSION), 0])
...
class IGEVStereo(nn.Module):
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        """ Estimate disparity between pair of frames the format of input tensor's shape is BCHW
        param image1: left image tensor (B, 3, H, W) 
        param image2: right image tensor (B, 3, H, W)
        """
...
class IGEVStereo_Onnx(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
    ...
    def forward(self, image1, image2):
        ...
        for itr in range(self.valid_iters):
            ...
            disp = disp + delta_disp       
            if self.test_mode and itr < self.valid_iters - 1:
                continue
