...
class BasicMotionEncoder_Onnx(nn.Module):
    def __init__(self, corr_levels,corr_radius):
        super(BasicMotionEncoder_Onnx, self).__init__()
        self.corr_levels  = corr_levels
        self.corr_radius = corr_radius
        cor_planes = self.corr_levels * (2*self.corr_radius + 1) * (8+1)
        self.convc1 = nn.Conv2d(cor_planes, 64, 1, padding=0)
        self.convc2 = nn.Conv2d(64, 64, 3, padding=1)
        self.convd1 = nn.Conv2d(1, 64, 7, padding=3)
        self.convd2 = nn.Conv2d(64, 64, 3, padding=1)
...
class BasicMultiUpdateBlock_Onnx(nn.Module):
    def __init__(self,corr_levels,corr_radius,n_gru_layers,n_downsample,hidden_dims=[]):
        super().__init__()
        self.n_gru_layers = n_gru_layers
        self.n_downsample = n_downsample
        self.hidden_dims = hidden_dims
        self.encoder = BasicMotionEncoder_Onnx(corr_levels,corr_radius)
        encoder_output_dim = 128
