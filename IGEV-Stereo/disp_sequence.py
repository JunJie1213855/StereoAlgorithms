...
def main(args:argparse.Namespace):
    ...
    model.eval()
    # 运行
    for i in tqdm(range(args.start_index,len(left_image_path))):
        left_frame = cv2.imread(left_image_path[i])
        right_frame = cv2.imread(right_image_path[i])
        ## 依靠参数文件的图像尺寸resize
        ...
        if args.rgbd_save ==1:
            if not os.path.exists(os.path.join(args.output_directory,"left")):
                os.mkdir(os.path.join(args.output_directory,"left"))
            if not os.path.exists(os.path.join(args.output_directory,"right")):
                os.mkdir(os.path.join(args.output_directory,"right"))
            if not os.path.exists(os.path.join(args.output_directory,"disp")):
                os.mkdir(os.path.join(args.output_directory,"disp"))
            # rgb图像保存
            cv2.imwrite(os.path.join(os.path.join(args.output_directory,"left"),f"{i}.jpg"),left_frame)
            # right
            cv2.imwrite(os.path.join(os.path.join(args.output_directory,"right"),f"{i}.jpg"),right_frame)
            # disp图像保存
            cv2.imwrite(os.path.join(os.path.join(args.output_directory,"disp"),f"{i}.pfm"),disp)
        else:
...
if __name__ == '__main__':
    ...
    ## 添加的选项
    parser.add_argument('--rgbd_save', help="directory to save output", default=0,type=int)
    parser.add_argument('--output_directory', help="directory to save output", default="E:/code/python/IGEV_stereo/IGEV-Stereo/stereo")
    parser.add_argument("--showrecitied",default=0,help="是否显示矫正的左右图")
    
    parser.add_argument("--param_file",default="./param/zed.yaml")
    parser.add_argument("-l","--left_sequence",default="E:\\dataset\\slam\\visual\\Zed\\dataset\\sequence\\teacher\\left\\")
    parser.add_argument("-r","--right_sequence",default="E:\\dataset\\slam\\visual\\Zed\\dataset\\sequence\\teacher\\right\\")

    parser.add_argument("-s","--start_index",default=1,type=int,help="起始")
    ## 命令行参数
