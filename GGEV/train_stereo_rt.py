...
from visualization import disp_error_img, save_images
...
def train(args):
    ...
    model = nn.DataParallel(GGEVStereo(args))
    ...
    while should_keep_training:
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            agg_pred, iter_preds, depth = model(image1, image2, iters=args.train_iters)
            assert model.training

            loss, metrics = sequence_loss(agg_pred, iter_preds, disp_gt, valid)
            logger.writer.add_scalar("train/live_loss", loss.item(), global_batch_num)
            logger.writer.add_scalar(f'train/learning_rate', optimizer.param_groups[0]['lr'], global_batch_num)
            global_batch_num += 1
            ...
            if total_steps % validation_frequency == validation_frequency - 1:
                ...
                if 'sceneflow' in args.train_datasets:
                    results = validate_sceneflow(args, model.module, iters=args.valid_iters)
                elif 'kitti' in args.train_datasets:
                    results = validate_kitti(args, model.module, iters=args.valid_iters)
                elif 'middlebury' in args.train_datasets:
                    results = validate_middlebury(args, model.module, iters=args.valid_iters)
                elif 'eth3d' in args.train_datasets:
                    results = validate_eth3d(args, model.module, iters=args.valid_iters)
                else:
            ...
            if total_steps % args.visual_freq == 0:
                img_summary = dict()
                img_summary['left'] = image1.float()
                img_summary['right'] = image2.float()
                img_summary['gt_disp'] = disp_gt.squeeze(1).float()
                
                img_summary["agg_pred"] = agg_pred.squeeze(1).float()
                img_summary["depth"] = depth.squeeze(1).float()
                n_predictions = len(iter_preds)
                for k in range(n_predictions):
                    if k < 5 or k == n_predictions - 1:
                        img_summary[f"iter_preds_{k}"] = iter_preds[k].squeeze(1).float()

                pred_disp = iter_preds[-1].squeeze(1).float()
                img_summary['disp_error'] = disp_error_img(pred_disp.squeeze(1).float(), disp_gt.squeeze(1).float())
                save_images(logger.writer, 'train', img_summary, total_steps)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='ggev-stereo', help="name your experiment")
    ...
    # Training parameters
    parser.add_argument('--batch_size', type=int, default=12, help="batch size used during training.")
    parser.add_argument('--train_datasets', default='sceneflow', choices=['sceneflow', 'kitti', 'kitti_finetune', 'eth3d_train', 'eth3d_finetune'], help="training datasets.")
    ...
    parser.add_argument('--train_iters', type=int, default=11, help="number of updates to the disparity field in each forward pass.")
    parser.add_argument('--wdecay', type=float, default=.00001, help="Weight decay in optimizer.")
    parser.add_argument('--encoder', default='vits', help="DepthAnything V2 encoder.")
    
    # Validation parameters
    parser.add_argument('--valid_iters', type=int, default=8, help='number of disparity field updates during validation forward pass')
    ...
    parser.add_argument('--saturation_range', type=float, nargs='+', default=[0.7, 1.3], help='color saturation')
    ...
    # log
    parser.add_argument('--visual_freq', default=1000, type=int, help='validation frequency in terms of training steps')
