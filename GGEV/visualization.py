...
def save_images(logger, mode_tag, images_dict, global_step):
    ...
    for tag, values in images_dict.items():
        ...
        for idx, value in enumerate(values):
            ...
            if value.shape[1] == 1:
                gray = np.squeeze(value)
                min_val = gray.min()
                max_val = gray.max()
                gray_normalized = (gray - min_val) / (max_val - min_val)
                color = cv2.applyColorMap((gray_normalized * 255).astype(np.uint8), cv2.COLORMAP_JET)
                value = color.astype(np.float32) / 255.0
                value = np.expand_dims(value, axis=0).transpose(0, 3, 1, 2)
