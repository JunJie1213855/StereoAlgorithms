...
class DispInitOnnx(nn.Module):
    ...
    def __init__(self,
        ...
        super(DispInitOnnx, self).__init__()
    ...
    def forward(self,
        ...
        x_grid = torch.linspace(0, w - 1, w, device=device, dtype=feature.dtype)
        if self.use_positivity:
            mask = torch.triu(torch.ones((w, w), dtype=torch.int32, device=device), diagonal=1)
        else:
            mask = torch.zeros((w, w), dtype=torch.int32, device=device)
