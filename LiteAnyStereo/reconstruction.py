...
def inference(left_img: np.ndarray, right_img: np.ndarray, model, max_disp=192):
    """
    使用LiteAnyStereo模型进行立体匹配推理

    Args:
        left_img: 左图像 (BGR格式)
        right_img: 右图像 (BGR格式)
        model: LiteAnyStereo模型
        max_disp: 最大视差值

    Returns:
        disp: 视差图 (numpy数组)
    """
    print("Load LiteAnyStereo model for inference!")
    model.eval()
    ...
    with torch.no_grad():
        ...
        disp = model(left_tensor, right_tensor, max_disp=max_disp, test_mode=True)
        # 转换回numpy
        disp = disp.cpu().numpy()
...
def main(args: argparse.Namespace):
    ...
    # 4. 加载LiteAnyStereo模型
    print("Loading LiteAnyStereo model...")
    model = LiteAnyStereo()
    ...
    disp = inference(left_img_rect, right_img_rect, model, max_disp=args.max_disp)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='LiteAnyStereo 3D Reconstruction')
